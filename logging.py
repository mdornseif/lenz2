...
class Formatter:
    ...
    def format(self, record):
        ...
        try:
            ...
            record.message = '*** problem assembling message: %r %r %r' % (record.msg, record.args, vars(record))
...
class Filter:
    ...
    def filter(self, record):
        """
        Is the specified record to be logged? Returns 1 for yes, 0 for no.
        """
        if self.nlen == 0:
            return 1
...
def fileConfig(fname):
    ...
    try:
        ...
        for log in llist:
            ...
            logger = getLogger(qn)
            if qn in existing: existing.remove(qn)
